...
- `/activite creer` — ouvre un formulaire (titre, lieu de départ, stuff optionnel), puis une vue de composition des slots : un menu déroulant alimenté par le **pool de rôles** du serveur (on peut ajouter plusieurs fois le même rôle, paginé au-delà de 23 rôles) plus un bouton **Ajout manuel** (`CATEGORIE - description`) pour les compo exotiques. Ensuite un sélecteur de date/heure. Toute la création se déroule en **message éphémère** (visible uniquement par le créateur) ; l'embed d'inscription n'est publié dans le salon qu'une fois validé, avec un menu déroulant pour rejoindre un slot, une fil de discussion, et un roster de tous les slots avec les joueurs inscrits. Un slot `❓ Fill` est ajouté automatiquement à la fin. *(Aucune restriction)*
- `/activite pool afficher` — affiche le pool de rôles ; `/activite pool ajouter <label>` ajoute un rôle (ex. `🛡️ Tank incub`) ; `/activite pool supprimer <position>` en supprime un (avec confirmation) ; `/activite pool réordonner` réordonne le pool via un sélecteur visuel (choisir le rôle à déplacer, puis la position voulue, puis Enregistrer) ; `/activite pool vider` vide le pool (avec confirmation). *(Nécessite : permission Administrateur Discord)*
- Slots — n'importe qui peut réclamer un slot depuis le menu déroulant de l'embed, ou dans la fil avec `/activite rejoindre <slot>`. Un joueur ne peut tenir qu'un seul slot par activité ; plusieurs joueurs peuvent partager un slot. Choisir un autre slot alors qu'on est déjà inscrit déplace l'inscription automatiquement. `/activite quitter` libère ton slot, et le bouton **Fermer** fige l'embed. Les menus **Modifier** et **Fermer** ne sont visibles que par l'organisateur et les admins — **Modifier** permet aussi de réécrire la liste des slots (les inscriptions existantes sont conservées pour les slots identiques). L'embed est la source de vérité, donc les commandes de fil continuent de fonctionner après un redémarrage du bot (aucune base de données pour les activités, rien n'est traqué).
