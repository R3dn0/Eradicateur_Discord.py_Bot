...
class BalanceCog(commands.GroupCog, group_name=app_commands.locale_str("balance", key="balance_name")):  # type: ignore[call-arg]
    ...
    async def show(
        ...
        if member is None:
            ...
            template = await self.bot.translate(
                "balance_show_self", interaction.locale
            )
            embed = discord.Embed(
        ...
        if not await payout_config_service.check_pay_add_permission(interaction.user):
            ...
            if level == "leader":
                msg = await self.bot.translate(
                    "balance_leader_only", interaction.locale
                )
            else:
                msg = await self.bot.translate(
                    "balance_officer_only", interaction.locale
                )
            await interaction.response.send_message(msg, ephemeral=True)
            return
        ...
        embed = discord.Embed(
            description=template.replace("{joueur}", member.mention)
    ...
    async def list_(self, interaction: discord.Interaction) -> None:
        ...
        if not balances:
            msg = await self.bot.translate(
                "balance_list_empty", interaction.locale
            )
            await interaction.response.send_message(msg, ephemeral=True)
            return

        title = await self.bot.translate("balance_list_title", interaction.locale)
        line_template = await self.bot.translate(
            "balance_list_entry", interaction.locale
        )
    ...
    async def payer(
        ...
        if not await payout_config_service.check_pay_add_permission(interaction.user):
            ...
            if level_config.pay_add_permission_level == "leader":
                msg = await self.bot.translate(
                    "balance_payer_leader_only", interaction.locale
                )
            else:
                msg = await self.bot.translate(
                    "balance_payer_officer_only", interaction.locale
                )
            await interaction.response.send_message(msg, ephemeral=True)
            return
    ...
    async def ajouter(
        ...
        if not await payout_config_service.check_pay_add_permission(interaction.user):
            ...
            if level_config.pay_add_permission_level == "leader":
                msg = await self.bot.translate(
                    "balance_ajouter_leader_only", interaction.locale
                )
            else:
                msg = await self.bot.translate(
                    "balance_ajouter_officer_only", interaction.locale
                )
            await interaction.response.send_message(msg, ephemeral=True)
            return
        ...
        new_balance = await transaction_repo.get_balance(joueur.id)
        template = await self.bot.translate(
            "balance_ajouter_credited", interaction.locale
        )
        await interaction.response.send_message(
            template.replace("{montant}", f"{montant:,.0f}")
            .replace("{joueur}", joueur.mention)
