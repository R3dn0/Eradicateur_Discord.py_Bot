...
async def test_create_payout_returns_id(repo):
    payout_id = await repo.create_payout(
        bag_silvers=1_000_000.0,
        item_market_value=500_000.0,
        activity_cost=50_000.0,
        tax_market=0.02,
        tax_guild=0.10,
        tax_transport=0.03,
        amount_per_player=100_000.0,
        buyback_value=475_000.0,
...
async def test_create_payout_persists_row(repo):
    payout_id = await repo.create_payout(
        bag_silvers=2_000_000.0,
        item_market_value=1_000_000.0,
        activity_cost=100_000.0,
        tax_market=0.02,
        tax_guild=0.10,
        tax_transport=0.03,
        amount_per_player=250_000.0,
        buyback_value=950_000.0,
    ...
    assert row["bag_silvers"] == 2_000_000.0
    assert row["item_market_value"] == 1_000_000.0
    assert row["activity_cost"] == 100_000.0
    assert row["tax_market"] == 0.02
    assert row["tax_guild"] == 0.10
    assert row["tax_transport"] == 0.03
    assert row["participant_count"] == 2
    assert row["amount_per_player"] == 250_000.0
    assert row["buyback_value"] == 950_000.0
...
async def test_buyback_value_formula(repo):
    item_market_value = 30_600_000.0
    tax_market = 0.02
    tax_transport = 0.03
    expected = item_market_value * (1 - tax_market - tax_transport)
    ...
    row = await cursor.fetchone()
    assert row["buyback_value"] == 29_070_000.0
